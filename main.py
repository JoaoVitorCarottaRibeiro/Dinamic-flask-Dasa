...
@app.route("/")
def index():
    return render_template("index.html")  # menu inicial

# 1. Exibir remédios
@app.route("/remedios")
def rota_exibir_remedios():
    return render_template("remedios.html", remedios=remedios)

# 2. Buscar remédio
@app.route("/buscar", methods=["GET", "POST"])
def rota_buscar_remedio():
    resultado = None
    if request.method == "POST":
        nome = request.form['nome']
        resultado = busca_binaria(nome)  # usa sua busca_binaria
...
def rota_atualizar():
    ...
    if request.method == "POST":
        ...
        # procurar o remédio na lista
        encontrado = next((r for r in remedios if r["nome"].lower() == nome.lower()), None)
        ...
        if encontrado:
            if operacao == '+':
                ...
                # não deixar negativo
                nova_qtd = max(encontrado["quantidade"] - quantidade, 0)
                encontrado["quantidade"] = nova_qtd
                tipo_op = "consumo"

            # registrar na fila/histórico
            registrar_consumo(encontrado["nome"], quantidade, tipo_op)

            mensagem = f"Remédio {encontrado['nome']} atualizado! Nova quantidade: {encontrado['quantidade']}."
        else:
            mensagem = f"Remédio '{nome}' não encontrado."

    # sempre retorna o mesmo template, com mensagem e lista atualizada
    return render_template("atualizar.html", remedios=remedios, mensagem=mensagem)
...
def rota_inserir():
    ...
    if request.method == "POST":    
        ...
        mensagem = f"Remédio {nome} inserido com sucesso!"

    return render_template("inserir.html", remedios=remedios, mensagem=mensagem)
    ...
    if request.method == "POST":
        ...
        for remedio in remedios:
            if remedio["codigo"] == codigo:
                ...
                historico_acoes.append(f"Remédio removido: {remedio['nome']} (Código: {remedio['codigo']})")
                mensagem = f"Remédio '{remedio['nome']}' removido com sucesso!"
                break
        else:
            mensagem = "Remédio não encontrado."
...
def rota_solicitar():
    ...
    if request.method == "POST":
        nome = request.form['nome']
        quantidade = int(request.form['quantidade'])
        encontrado = next((r for r in remedios if r["nome"].capitalize() == nome.capitalize()), None)
        if encontrado:
            encontrado["quantidade"] += quantidade
            registrar_consumo(nome, quantidade, "adição")
            mensagem = f"Lote adicionado ao remédio '{encontrado['nome']}' com sucesso!"
        else:
            mensagem = f"Remédio '{nome}' não encontrado."
    return render_template("solicitar.html", remedios=remedios, mensagem=mensagem)

# 7. Listar remédios próximos à validade
@app.route("/proximos_validade")
def rota_proximos_validade():
    # aqui transformei listar_proximos_validade para devolver uma lista
    proximos = []
    import datetime
    ...
    for r in remedios:
        ...
        if 0 <= diferenca <= meses:
            proximos.append(r)
    return render_template("proximos_validade.html", proximos=proximos)

# 8. Histórico de ações
@app.route("/historico")
def rota_historico():
    return render_template("historico.html", historico=historico_acoes)
...
def rota_alertas():
    ...
    if request.method == "POST":
        ...
        sucesso, mensagem = alertas_email(email_usuario)
    return render_template("alertas.html", mensagem=mensagem)

# 10. Consultar fila de consumos
@app.route("/fila")
def rota_fila():
    return render_template("fila.html", fila=fila_consumo)

# 11. Consultar pilha de últimas consultas
@app.route("/pilha", methods=["GET", "POST"])
def rota_pilha():
    if request.method == "POST":
        remedio_nome = request.form.get("remedio")
        if remedio_nome:
            registrar_consulta(remedio_nome)
        return redirect(url_for("rota_pilha"))
    return render_template("pilhas.html", pilha=pilha_consultas)

# 12. Ordenar remédios por validade (Merge Sort)
@app.route("/ordenar")
def rota_ordenar():
    merge_sort_validade(remedios)
    return render_template("ordenar.html", remedios=remedios)

# 13. Gerar relatório PDF
@app.route("/relatorio")
def rota_relatorio():
    acao = request.args.get("acao")
    if acao == "pdf":
        if not historico_acoes:
            return render_template("relatorio.html", aviso="Nenhuma ação registrada para gerar relatório.")
...
# Função para registrar consumo, nela podemos observar o requisito de fila sendo utilizado. Ela também faz parte do histórico de ações do sistema. O registro é feito por meio de um dicionário, contendo as informações como data de adição, nome do remédio, quantidade e o tipo de operação.
...
def alertas_email(email_destino):
    ...
    y = 780
    for acao in historico_acoes:
        linhas = textwrap.wrap(acao, width=100)
        for linha in linhas:
    ...
    try:
        ...
        server.quit()
        return True, f"E-mail enviado com sucesso para {email_destino}!"
...
def gerar_relatorio_pdf():
    ...
    nome_arquivo = "Relatorio_Acoes.pdf"
